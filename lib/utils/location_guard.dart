...
class LocationGuard {
  ...
  /// ت٣: مسجّل واقعة الموقع المُحاكى — يضبطه تطبيق الكابتن عند الإقلاع
  /// (عدّاد على مستنده + سطر في سجلّ الإدارة). الحارس أداة خالصة بلا
  /// Provider فيُحقن المسجّل حقناً؛ null = لا تسجيل (نكهات أخرى).
  static void Function()? onMockedLocation;
  ...
  static Future<String?> checkNear({
    ...
    if (pos.isMocked) {
      // ت٣: الرفض وحده كان يُبخّر أرخص إشارة احتيال في النظام — الكابتن
      // يطفئ تطبيق الموقع الوهمي ويعيد المحاولة ولا يصل المدير شيء.
      // «أطلق وانسَ»: التسجيل لا يؤخّر الرسالة ولا يكسرها إن فشل.
      onMockedLocation?.call();
